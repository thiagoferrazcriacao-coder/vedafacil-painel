...
// ── LocalCard — local do orçamento com upload de fotos ───────────────────────
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
        {[
          { campo: 'trinca',     label: 'Trinca',     un: 'm'   },
          { campo: 'juntaFria',  label: 'Junta Fria', un: 'm'   },
          { campo: 'ralo',       label: 'Ralo',       un: 'un'  },
          { campo: 'juntaDilat', label: 'Jt. Dilat.', un: 'm'   },
          { campo: 'ferragem',   label: 'Ferragem',   un: 'm'   },
          { campo: 'cortina',    label: 'Cortina',    un: 'm²'  },
        ].map(({ campo, label, un }) => (
          <div key={campo} className="flex flex-col">
            <span className="text-xs text-gray-400">{label} <span className="text-gray-300">({un})</span></span>
            <input type="number" step="0.1" min="0"
              className="border border-gray-200 rounded px-1.5 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-[#e87722]"
              value={local[campo] ?? ''}
              onChange={e => onChange(idx, campo, e.target.value === '' ? null : Number(e.target.value))}
            />
          </div>
        ))}
      </div>
  ...
  async function criarRegistros() {
    ...
    try {
      const dadosOrcamento = {
        ...orcDados,
        locais: locais.map(l => ({ ...l, fotos: undefined })),
      }
      // Etapa 1 — cria sem fotos
